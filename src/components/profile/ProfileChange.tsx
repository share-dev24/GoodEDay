...
export default function ProfileChange() {
  ...
  const { displayName, uid, photo, setUser } = useUserStore<UserState>(
    (state) => ({
      ...
      <div className='flex justify-between items-center pr-[20px] border-b-2 border-solid border-b-gray'>
        <div className='flex  gap-[20px] p-[20px] '>
          ...
          <div className='flex items-center gap-6px'>
            ...
            <div
              className={`flex flex-col group max-w-sm space-y-[12px] ${!isInputOpen && 'hidden'}`}
            >
              <input
                type='text'
            ...
            <button
              ...
              onClick={handleNameChangeButton}
            >
              <svg
        ...
        <button
          ...
          className='text-[12px]  w-[100px] h-[30px]  text-primary border border-primary rounded-lg p-4px hover:text-white hover:bg-primary'
        >
          프로필사진수정
        </button>
      </div>
